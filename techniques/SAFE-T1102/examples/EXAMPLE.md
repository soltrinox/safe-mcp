# SAFE-T1102: Prompt Injection (Multiple Vectors) - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1102: Prompt Injection (Multiple Vectors)** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify prompt injection (multiple vectors) indicators in MCP environments.
...
```bash
python3 detector.py safe-t1102_attack_logs.json
```
