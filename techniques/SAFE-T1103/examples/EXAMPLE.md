# SAFE-T1103: Fake Tool Invocation (Function Spoofing) - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1103: Fake Tool Invocation (Function Spoofing)** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
In normal MCP operations, tools are registered by servers and then invoked by hosts through standardized JSON-RPC 2.0 messages. However, if the h
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify fake tool invocation (function spoofing) indicators in MCP environments.
...
```bash
python3 detector.py safe-t1103_attack_logs.json
```
