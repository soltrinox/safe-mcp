# SAFE-T2102: Service Disruption via External API Flooding - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T2102: Service Disruption via External API Flooding** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify service disruption via external api flooding indicators in MCP environments.
...
```bash
python3 detector.py safe-t2102_attack_logs.json
```
