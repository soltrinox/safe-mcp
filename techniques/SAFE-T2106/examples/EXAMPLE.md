# SAFE-T2106: Context Memory Poisoning via Vector Store Contamination - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T2106: Context Memory Poisoning via Vector Store Contamination** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify context memory poisoning via vector store contamination indicators in MCP environments.
...
```bash
python3 detector.py safe-t2106_attack_logs.json
```
