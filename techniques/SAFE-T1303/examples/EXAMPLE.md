# SAFE-T1303: Container Sandbox Escape via Runtime Exec - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1303: Container Sandbox Escape via Runtime Exec** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify container sandbox escape via runtime exec indicators in MCP environments.
...
```bash
python3 detector.py safe-t1303_attack_logs.json
```
