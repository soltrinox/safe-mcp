# SAFE-T1705: Cross-Agent Instruction Injection - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1705: Cross-Agent Instruction Injection** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify cross-agent instruction injection indicators in MCP environments.
...
```bash
python3 detector.py safe-t1705_attack_logs.json
```
