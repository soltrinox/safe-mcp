# SAFE-T1601: MCP Server Enumeration - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1601: MCP Server Enumeration** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify mcp server enumeration indicators in MCP environments.
...
```bash
python3 detector.py safe-t1601_attack_logs.json
```
