# SAFE-T2107: AI Model Poisoning via MCP Tool Training Data Contamination - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T2107: AI Model Poisoning via MCP Tool Training Data Contamination** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify ai model poisoning via mcp tool training data contamination indicators in MCP environments.
...
```bash
python3 detector.py safe-t2107_attack_logs.json
```
