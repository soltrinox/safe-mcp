# SAFE-T1501: Full-Schema Poisoning (FSP) - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1501: Full-Schema Poisoning (FSP)** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify full-schema poisoning (fsp) indicators in MCP environments.
...
```bash
python3 detector.py safe-t1501_attack_logs.json
```
