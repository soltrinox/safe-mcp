#!/usr/bin/env python3
"""
SAFE-T1501: Full-Schema Poisoning (FSP) - Detection Implementation

This detector identifies indicators of full-schema poisoning (fsp) attacks in MCP environments.
...
class T1501Detector:
    """Detector for Full-Schema Poisoning (FSP) attacks in MCP environments"""
    ...
    def scan(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        """Scan for full-schema poisoning (fsp) indicators"""
        findings = {
            'critical': [],
            'high': [],
            'medium': [],
    ...
    def scan_logs(self, log_file: str) -> Dict[str, Any]:
        """Scan log file for full-schema poisoning (fsp) indicators"""
        try:
    ...
    def generate_report(self, scan_results: Dict[str, Any]) -> str:
        ...
        report = f"\n=== SAFE-T1501 Detection Report ===\n\n"
        report += f"Technique: Full-Schema Poisoning (FSP)\n"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(f"\nSAFE-T1501: Full-Schema Poisoning (FSP) Detector")
