# SAFE-T1504: Token Theft via API Response - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1504: Token Theft via API Response** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify token theft via api response indicators in MCP environments.
...
```bash
python3 detector.py safe-t1504_attack_logs.json
```
