# SAFE-T1105: Path Traversal via File Tool - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1105: Path Traversal via File Tool** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
MCP file tools (such as file readers, writers, or directory browsers) that accept relative paths without proper validation can be exploited to traverse outside their intended directory boundaries. Attackers can use path traversal sequences like
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify path traversal via file tool indicators in MCP environments.
...
```bash
python3 detector.py safe-t1105_attack_logs.json
```
