# SAFE-T1110: Multimodal Prompt Injection via Images/Audio - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1110: Multimodal Prompt Injection via Images/Audio** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify multimodal prompt injection via images/audio indicators in MCP environments.
...
```bash
python3 detector.py safe-t1110_attack_logs.json
```
