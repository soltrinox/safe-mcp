# SAFE-T1007: OAuth Authorization Phishing - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1007: OAuth Authorization Phishing** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify oauth authorization phishing indicators in MCP environments.
...
```bash
python3 detector.py safe-t1007_attack_logs.json
```
