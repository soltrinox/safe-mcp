# SAFE-T1001: Tool Poisoning Attack (TPA) - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1001: Tool Poisoning Attack (TPA)** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify tool poisoning attack (tpa) indicators in MCP environments.
...
```bash
python3 detector.py safe-t1001_attack_logs.json
```
