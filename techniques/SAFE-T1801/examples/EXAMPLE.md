# SAFE-T1801: Automated Data Harvesting - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1801: Automated Data Harvesting** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The attack leverages the agent's ability to programmatically iterate and call tools. An adversary, typically through prompt injection, provides
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify automated data harvesting indicators in MCP environments.
...
```bash
python3 detector.py safe-t1801_attack_logs.json
```
