# SAFE-T1202: OAuth Token Persistence - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1202: OAuth Token Persistence** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify oauth token persistence indicators in MCP environments.
...
```bash
python3 detector.py safe-t1202_attack_logs.json
```
