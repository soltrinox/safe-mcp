# SAFE-T1101: Command Injection - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1101: Command Injection** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify command injection indicators in MCP environments.
...
```bash
python3 detector.py safe-t1101_attack_logs.json
```
