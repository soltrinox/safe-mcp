# SAFE-T1003: Malicious MCP-Server Distribution - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1003: Malicious MCP-Server Distribution** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify malicious mcp-server distribution indicators in MCP environments.
...
```bash
python3 detector.py safe-t1003_attack_logs.json
```
