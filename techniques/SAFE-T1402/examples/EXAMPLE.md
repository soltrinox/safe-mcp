# SAFE-T1402: Instruction Stenography - Tool Metadata Poisoning - Example Implementation

## Overview

This directory contains working examples demonstrating **SAFE-T1402: Instruction Stenography - Tool Metadata Poisoning** detection and attack simulation. These examples are generated from `pseudocode.md` and `detection-rule.yml`.
...
In the context of MCP (Model Context Protocol), attackers exploit the structured and textual nature of tool configurations by injecting zero-width Unicode characters, HTML comme
...
The detector (`detector.py`) implements patterns from the Sigma detection rule to identify instruction stenography - tool metadata poisoning indicators in MCP environments.
...
```bash
python3 detector.py safe-t1402_attack_logs.json
```
